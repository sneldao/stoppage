...
import { useOddsHistory } from "@/lib/odds/useOddsHistory";
...
export function OddsSparkline({ marketId, currentYes, width = 120, height = 32 }: OddsSparklineProps) {
  ...
  const inView = useInViewport(rootRef, { rootMargin: "120px" });
  const points = useOddsHistory(marketId, inView);
  ...
  if (!path) {
    return (
      <div ref={rootRef} className="odds-sparkline odds-sparkline-empty" style={{ width, height }} title="Odds history building">
  ...
  return (
    <div ref={rootRef} className="odds-sparkline" style={{ width, height, position: "relative" }} title={`Odds moved ${delta >= 0 ? "+" : ""}${(delta * 100).toFixed(0)}%`}>
